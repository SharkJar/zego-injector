...
const proxy = function (propertyName:string,target: any,propConfig:propertyFactory | propertyData){
    ...
    let getter:Function
    // 删除原属性
    if(!(delete target.prototype[propertyName])){ return }
    // 只有删除成功之后 在附加
...
export const initProperty = function (target: any) {
    ...
    if(!Array.isArray(propertyParams)){ return }
    // 做代理
    propertyParams.forEach((propertyConfig:propertyFactory | propertyData) => proxy(propertyConfig.propertyName || "",target,propertyConfig));
};