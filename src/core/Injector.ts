...
class InjectHelper{
    ...
    private createModule<TOutput = any>(provide: any){
        ...
        let instance
        // 通过构造函数创建
        if(mod.constructor){ instance = this.createConstructor(mod.constructor,mod.provide,mod.inject) }
        // 通过factory创建
        if(mod.createFunction){ instance = this.createFactory(mod.createFunction,mod.provide,mod.inject) }
        // 返回
    ...
    public useClass(params:useClassParams){
        ...
        // 已经存在
        if(this.modules.has(provide)){ return }
        // 拿到构造函数
    ...
    public create<TOutput = any>(creator: constructor | any){
        ...
        if(mod.constructor){ return this.createConstructor(mod.constructor,mod.provide,mod.inject) }
        // 如果module是createFunction模式
        if(mod.createFunction){ return this.createFactory(mod.createFunction,mod.provide,mod.instance) }
    }
    ...
    public createConstructor<TOutput = any>(creator: constructor | any,provide?: any,inject?: any[]){
        ...
        // 拿到module 进行创建使用 
        const mod:module = this.modules.get(provide) || { inject:Array.isArray(inject)? inject : this.getConstructorParamTypes(creator),constructor:creator,provide }
    ...
    public createFactory<TOutput = any>(createFunction: Function,provide: any,inject?: any[]){
        // 拿到module 进行创建使用 
        const mod:module = this.modules.get(provide) || { inject:Array.isArray(inject)? inject : [],createFunction,provide,constructor:undefined }
        // 已经存在实例
        if(mod.instance){ return mod.instance }
        // 返回创建对象
        const paramsInstance = (mod.inject || []).map(this.createModule.bind(this))
