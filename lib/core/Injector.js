...
var InjectHelper = (function () {
    ...
    InjectHelper.prototype.createConstructor = function (creator, provide, inject) {
        ...
        var mod = this.modules.get(provide) || { inject: Array.isArray(inject) ? inject : this.getConstructorParamTypes(creator), constructor: creator, provide: provide };
        if (mod.instance) {
            return mod.instance;
        }
        initProperty_1.initProperty(creator);
        var paramsInstance = (mod.inject || []).map(this.createModule.bind(this));
        mod.instance = new (creator.bind.apply(creator, __spreadArrays([void 0], paramsInstance)))();
        this.modules.set(provide, mod);
        return mod.instance;
    };
    InjectHelper.prototype.createFactory = function (createFunction, provide, inject) {
        var mod = this.modules.get(provide) || { inject: Array.isArray(inject) ? inject : [], createFunction: createFunction, provide: provide, constructor: undefined };
        if (mod.instance) {
            return mod.instance;
        }
        var paramsInstance = (mod.inject || []).map(this.createModule.bind(this));
